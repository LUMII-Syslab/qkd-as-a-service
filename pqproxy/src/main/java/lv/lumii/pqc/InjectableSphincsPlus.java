...
public class InjectableSphincsPlus implements SigAlgAPI {
    ...
    private static final SPHINCSPlusParameters sphincsPlusParameters = SPHINCSPlusParameters.sha2_128f;//.sha2_128f_simple;
    ...
    public InjectableSphincsPlus() {
        this(//"SPHINCS+",
                "SPHINCS+-SHA2-128F",
                //List.of(new String[]{"SPHINCS+", "SPHINCSPLUS"}),
                List.of(new String[]{}),
    ...
    public InjectableSphincsPlus(String name, Collection<String> aliases, ASN1ObjectIdentifier oid, int sigCodePoint) {
        this.name = name;
    ...
    {
        return key instanceof BCSPHINCSPlusPublicKey;
    }

    @Override
    public boolean isSupportedPrivateKey(Key key)
    {
        return key instanceof BCSPHINCSPlusPrivateKey;
    }
    ...
    public byte[] internalEncodingFor(PublicKey key) {
        ...
        byte[] b4 = SPHINCSPlusParameters.sha2_128f.getEncoded(); //sha2_128f_simple
    ...
    {
        BCSPHINCSPlusPrivateKey sk = (BCSPHINCSPlusPrivateKey) key;
    ...
    public SignatureSpi signatureSpi(Key publicOrPrivateKey) {
        ...
        nameMatches = nameMatches || "SPHINCS+".equals(publicOrPrivateKey.getAlgorithm());
        nameMatches = nameMatches || "SPHINCSPLUS".equals(publicOrPrivateKey.getAlgorithm());

        // TODO: check if we support the parameters of the given key
